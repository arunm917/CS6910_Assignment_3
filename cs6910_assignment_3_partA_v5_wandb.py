# -*- coding: utf-8 -*-
"""CS6910_Assignment_3_PartA_WANDB
...
class Seq2Seq(nn.Module):
    def __init__(self, encoder, decoder, cell_type_encoder, cell_type_decoder, bidirectionality, num_layers, hidden_size, teacher_forcing_ratio):
...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size, shuffle=True, num_workers = 25)
val_loader = torch.utils.data.DataLoader(val_dataset, batch_size, shuffle=True, num_workers = 25)
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size, shuffle=True, num_workers = 25)
...
def train(num_epochs, model, optimizer, criterion, scheduler):
  for epoch in tqdm(range(num_epochs)):
      ...
      wandb.log({'loss_epoch': loss_epoch, 'Training accuracy':train_accuracy, 
...
def wandbsweeps():
  ...
  decoder_net = Decoder(input_size_decoder,wandb.config.embedding_size, wandb.config.hidden_size,
                        ...
                        wandb.config.cell_type_decoder).to(device)
  model = Seq2Seq(encoder_net, decoder_net, wandb.config.cell_type_encoder, wandb.config.cell_type_decoder, 
                  wandb.config.bidirectionality, wandb.config.num_layers, wandb.config.hidden_size, 
                  wandb.config.teacher_forcing_ratio).to(device)
